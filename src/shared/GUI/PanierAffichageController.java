...
//        try {
    ...
    public double AffTotal(){
        ...
        for (int i=0; i < prods.ListPanierDetailsUser(connectedUser.getUser().getCin()).size(); i++){
            double total1 = prods.ListPanierDetailsUser(connectedUser.getUser().getCin()).get(i).getPrix() ;
            System.out.println(prods.ListPanierDetailsUser(connectedUser.getUser().getCin()).get(i));
            System.out.println(prods.ListPanierDetailsUser(connectedUser.getUser().getCin()).get(i).getPrix());
    ...
    private void AfficherProdHori() throws SQLException, IOException {
        ...
        int column = 0;
        int row = 1;
        System.out.println(prods.getProdJoin(connectedUser.getUser().getCin()).size());
        
        try {
            for (int i = 0; i < prods.getProdJoin(connectedUser.getUser().getCin()).size(); i++) {
                sup = new Suppression() {
                    @Override
                ...
                ProduitServ.AddProduit(prods.getProdJoin(connectedUser.getUser().getCin()).get(i), myListener, sup);
